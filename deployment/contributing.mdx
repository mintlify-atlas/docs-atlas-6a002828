...
<Steps>
  ...
  <Step title="Verify the build passes">
    Run a production build to catch TypeScript errors and ensure nothing is broken before opening a PR:
...
<Tip>
  Run `npm run build` before pushing your branch. The build step runs the TypeScript compiler across the entire project and will surface type errors that the dev server may have silently ignored. Catching these locally saves a round-trip in review.
</Tip>
