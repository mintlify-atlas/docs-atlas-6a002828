---
title: "SpinAI Database Schema: Tables and Relationships"
sidebarTitle: "Schema"
...
<Note>
  After the initial migration, run `20260702180000_agenda_key_points_a_jsonb.sql` to convert `agenda` and `key_points` from `text[]` to `jsonb`. The migration is idempotent — it checks the current column type before altering, so it is safe to run on a database where the columns are already `jsonb`.
</Note>
